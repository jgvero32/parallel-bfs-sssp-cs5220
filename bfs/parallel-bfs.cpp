...
void parallel_bfs(const Graph &g, int rank, int num_procs) {
    ...
    while (true) {
        ...
        MPI_Alltoallv(new_frontier.data(), rows_per_proc.data(),
                      displacements.data(), MPI_INT, recv_vals.data(),
                      row_ct_per_rank, displacements_rank, MPI_INT,
                      MPI_COMM_WORLD);
        ...
        for (int row = start_row; row < end_row; ++row) {
            ...
            for (int i = 0; i < num_procs; ++i) {
                ...
                if (recv_vals[i * (end_row - start_row) + offset] == 1 &&
                    dists[row] == -1) {
                    frontier[row] = true;
                    dists[row] = distance;
                    has_frontier = 1;
        ...
        int frontier_nodes;
        MPI_Allreduce(&has_frontier, &frontier_nodes, 1, MPI_INT, MPI_SUM,
                      MPI_COMM_WORLD);
